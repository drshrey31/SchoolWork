...
let moves (s:state) : state list =
  ...
  in
  let move_wolf (p,w,g,c) = [ ( other_side p, other_side w, g, c) ]
  in
  let move_goat (p,w,g,c) = [ ( other_side p, w, other_side g, c) ]
  in
  let move_cabbage (p,w,g,c) = [ ( other_side p, w, g, other_side c) ]
  in
