(* Leftists heaps.
   Sometimes called queues.  Useful for collections
   for which one needs to know a minimal value.
 *)

 type 'a heap = E
 | T of int * 'a * 'a heap * 'a heap

let rank (t: 'a heap) : int =
match t with
| E -> 0
| T (r, _, _, _) -> r

let makeT (x: 'a) (a: 'a heap) (b: 'a heap) : 'a heap =
if rank a >= rank b
then T (rank b + 1, x, a, b)
else T (rank a + 1, x, b, a)
(* makeT is not a general insert function, it assumes that
x is the smallest element in the resulting heap. *)
...
if x <= y
then makeT x a1 (merge b1 t2)
else makeT y a2 (merge t1 b2)

(* merge runs in O(log n) *)

let insert (x: 'a) (h: 'a heap) : 'a heap =
merge (T (1, x, E, E)) h

let findMin (h: 'a heap) : 'a =
match h with
| E -> failwith "empty heap in findMin"
| T (_, x, _, _) -> x

let deleteMin (h: 'a heap) : 'a heap =
match h with
| E -> failwith "empty heap in deleteMin"
| T (_, _, a, b) -> merge a b

let fromList (lst: 'a list) : 'a heap =
List.fold_left (fun h x -> insert x h) E lst
